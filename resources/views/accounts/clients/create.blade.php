    ...
    <div class="w-full">
        ...
        <div class="mb-6 flex flex-wrap items-start gap-4">
            ...
            <div class="min-w-0 flex-1">
                <h1 class="mb-1.5 text-2xl font-bold tracking-tight text-slate-800 dark:text-slate-100">Add Client Account</h1>
                <p class="text-slate-600 dark:text-slate-400">Create a new client account linked to a user code (unique_code from User Accounts).</p>
            </div>
        </div>
        ...
        <form action="{{ route('accounts.clients.store') }}" method="POST" class="space-y-6" autocomplete="off">
            @csrf
                    ...
                    <div class="space-y-4">
                        ...
                        <div class="grid gap-4 md:grid-cols-2">
                            <div>
                                ...
                                <input type="text" id="client_name" name="client_name" class="w-full rounded-lg border border-slate-300 bg-white px-4 py-2.5 text-sm text-slate-800 placeholder-slate-400 transition-colors focus:border-emerald-500 focus:outline-none focus:ring-2 focus:ring-emerald-500/25 dark:border-slate-600 dark:bg-slate-700 dark:text-slate-100 dark:placeholder-slate-500 dark:focus:ring-emerald-500/30" placeholder="Client display name" value="{{ old('client_name') }}" required>
                            </div>
                            <div>
                                ...
                                <input type="email" id="client_email" name="client_email" class="w-full rounded-lg border border-slate-300 bg-white px-4 py-2.5 text-sm text-slate-800 placeholder-slate-400 transition-colors focus:border-emerald-500 focus:outline-none focus:ring-2 focus:ring-emerald-500/25 dark:border-slate-600 dark:bg-slate-700 dark:text-slate-100 dark:placeholder-slate-500 dark:focus:ring-emerald-500/30" placeholder="client@example.com" value="{{ old('client_email') }}" required>
                            </div>
                        </div>
                    </div>
                </div>
...
@push('scripts')
    ...
    <script>
        (function() {
            var form = document.querySelector('form[action="{{ route('accounts.clients.store') }}"]');
            ...
            if (codeSelect && nameInput && emailInput) {
                codeSelect.addEventListener('change', function() {
                    ...
                    if (previewCode) previewCode.textContent = this.value || 'SHG001';
                    if (previewName && nameInput) previewName.textContent = nameInput.value || 'Sample Client';
                    if (previewEmail && emailInput) previewEmail.textContent = emailInput.value || 'client@example.com';
                });
            }
