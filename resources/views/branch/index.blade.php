    ...
    <div class="w-full">
        ...
        <div class="mb-6 flex flex-wrap items-start justify-between gap-4">
            <div class="flex items-start gap-4">
                ...
                <div class="min-w-0">
                    <h1 class="mb-1.5 text-2xl font-bold tracking-tight text-slate-800 dark:text-slate-100">Branch</h1>
                    <p class="text-slate-600 dark:text-slate-400">View and manage company branches used in LBS jobs.</p>
                </div>
            </div>
            <a href="{{ route('branch.create') }}" class="inline-flex items-center gap-2 rounded-xl bg-emerald-600 px-4 py-2.5 text-sm font-semibold text-white shadow-md transition-all hover:bg-emerald-500 focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:ring-offset-2 dark:focus:ring-offset-slate-900">
        ...
        <div class="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm dark:border-slate-700 dark:bg-slate-800/60">
            <div class="overflow-x-auto">
                <table class="w-full min-w-[720px] border-collapse text-sm" id="branchTable">
                    <thead>
                    ...
                    <tbody>
                        @forelse($branches as $branch)
                            <tr class="border-b border-slate-100 transition-colors hover:bg-slate-50 dark:border-slate-700 dark:hover:bg-slate-800/50">
                                ...
                                <td class="px-5 py-3.5 text-slate-600 dark:text-slate-400">{{ $branch->created_at?->format('M j, Y g:i A') ?? '—' }}</td>
                                <td class="px-5 py-3.5 text-slate-600 dark:text-slate-400">{{ $branch->updated_at?->format('M j, Y g:i A') ?? '—' }}</td>
                                <td class="px-5 py-3.5 text-right">
                                    <div class="flex items-center justify-end gap-1">
                                        <a href="{{ route('branch.edit', $branch) }}" class="inline-flex h-9 w-9 items-center justify-center rounded-lg text-slate-500 transition-colors hover:bg-emerald-500/15 hover:text-emerald-600 dark:text-slate-400 dark:hover:text-emerald-400" title="Edit" aria-label="Edit">
                                        ...
                                        <form action="{{ route('branch.destroy', $branch) }}" method="POST" class="inline" data-delete-form autocomplete="off">
                                            @csrf
                                            @method('DELETE')
                        ...
                        @empty
                            <tr>
                                <td colspan="5" class="px-5 py-12 text-center text-slate-500 dark:text-slate-400">
                                    <svg class="mx-auto mb-3 h-12 w-12 text-slate-300 dark:text-slate-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M3 7l9-4 9 4-9 4-9-4zm0 6l9 4 9-4m-9 4v6"/></svg>
                                    <p class="font-medium">No branch records yet.</p>
                                    <p class="mt-1 text-sm"><a href="{{ route('branch.create') }}" class="text-emerald-600 hover:underline dark:text-emerald-400">Add one</a> to get started.</p>
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
        ...
        <div class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4 opacity-0 pointer-events-none transition-opacity duration-200 backdrop-blur-sm" id="deleteBranchModal" role="dialog" aria-labelledby="deleteBranchModalTitle" aria-modal="true">
            <div class="w-full max-w-sm overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-xl dark:border-slate-600 dark:bg-slate-800" role="document">
                <div class="flex items-center gap-3 px-5 py-5">
                    ...
                    <h2 class="text-lg font-bold text-slate-800 dark:text-white" id="deleteBranchModalTitle">Delete Branch</h2>
                </div>
                <div class="px-5 pb-4">
                    <div id="deleteBranchModalConfirmBlock">
                        <p class="text-slate-600 dark:text-slate-300">Are you sure you want to delete this branch? This action cannot be undone.</p>
                    </div>
                    <div class="hidden" id="deleteBranchModalCountdown">
                        <p class="text-sm font-medium text-slate-600 dark:text-slate-400">Deleting in</p>
                        <div class="mt-2 text-2xl font-bold text-red-600 dark:text-red-400" id="deleteBranchCountdownNumber">3</div>
                ...
                <div class="flex justify-end gap-3 border-t border-slate-200 px-5 py-4 dark:border-slate-700">
                    <button type="button" class="rounded-lg border border-slate-300 bg-white px-4 py-2.5 text-sm font-medium text-slate-700 transition-colors hover:bg-slate-50 focus:outline-none focus:ring-2 focus:ring-slate-400 focus:ring-offset-2 dark:border-slate-600 dark:bg-slate-700 dark:text-slate-200 dark:hover:bg-slate-600 dark:focus:ring-offset-slate-800" id="deleteBranchModalCancel">Cancel</button>
                    <button type="button" class="rounded-lg bg-red-600 px-4 py-2.5 text-sm font-medium text-white transition-colors hover:bg-red-500 focus:outline-none focus:ring-2 focus:ring-red-500 focus:ring-offset-2 dark:focus:ring-offset-slate-800" id="deleteBranchModalConfirm"><span class="btn-text">Delete</span></button>
                </div>
            </div>
        </div>
    </div>
