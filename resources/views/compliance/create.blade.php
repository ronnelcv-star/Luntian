    ...
    <div class="w-full">
        ...
        <div class="mb-6 flex flex-wrap items-start gap-4">
            ...
            <div class="min-w-0 flex-1">
                <h1 class="mb-1.5 text-2xl font-bold tracking-tight text-slate-800 dark:text-slate-100">Add Compliance</h1>
                <p class="text-slate-600 dark:text-slate-400">Create a new NCC compliance record. This value will appear in job request dropdowns.</p>
            </div>
        </div>
        ...
        <form action="{{ route('compliance.store') }}" method="POST" class="space-y-6" autocomplete="off">
            @csrf
            <div class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-700 dark:bg-slate-800/60">
                <div class="space-y-4">
                    <div>
                        <label for="column" class="mb-1.5 block text-sm font-medium text-slate-700 dark:text-slate-300">Compliance name</label>
                        <input type="text" id="column" name="column" value="{{ old('column') }}" placeholder="e.g. NCC 2022, WOH 2022" autocomplete="off"
...
@push('scripts')
    <script>
        (function() {
            var form = document.querySelector('form[action="{{ route('compliance.store') }}"]');
